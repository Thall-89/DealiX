...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <FinancialSummary builds={builds} />

      <section className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-[28px] border border-white/10 bg-slate-950/40 p-6"><h2 className="text-xl font-semibold text-white">Completed sales</h2><div className="mt-4 space-y-3">{builds.filter((build) => build.status === "Sold").map((build) => { const financials = confirmedBuildFinancials(build); return <div key={build.id} className="rounded-2xl border border-emerald-400/20 bg-emerald-500/10 p-4 text-sm text-emerald-100"><div className="font-medium text-white">{build.name}</div><div className="mt-2 grid gap-1 sm:grid-cols-2"><span>Sold price: ${financials.soldPrice.toFixed(2)}</span><span>Actual payout: ${financials.payout.toFixed(2)}</span><span>Total invested: ${financials.totalInvested.toFixed(2)}</span><span>Confirmed profit: ${financials.netProfit.toFixed(2)}</span><span>ROI: {financials.roi?.toFixed(2)}%</span><span>Sale date: {build.saleDate ?? "Not entered"}</span></div></div>; })}</div></div>
        <div className="rounded-[28px] border border-white/10 bg-slate-950/40 p-6"><h2 className="text-xl font-semibold text-white">Active investments</h2><div className="mt-4 space-y-3">{builds.filter((build) => build.status !== "Sold").map((build) => { const projection = projectedBuildFinancials(build); return <div key={build.id} className="rounded-2xl border border-amber-400/20 bg-amber-500/10 p-4 text-sm text-amber-100"><div className="font-medium text-white">{build.name}</div><div className="mt-2">Current cost: ${build.buildCost.toFixed(2)} · Expected sale: {build.expectedSale ?? build.estimatedResale ?? "Not entered"}</div><div className="mt-1">Projected profit: {projection ? `$${projection.lowProfit.toFixed(2)} – $${projection.highProfit.toFixed(2)}` : "Not entered"}</div><div className="mt-1 text-amber-200">{projection?.incomplete ? "Incomplete projection: selling expenses or remaining costs are missing." : "Projected only."}</div></div>; })}</div></div>
      </section>
