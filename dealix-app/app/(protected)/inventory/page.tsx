...
const statuses = ["All", "Available", "Assigned to Build", "Installed in Personal PC", "Needs Parts", "Needs Testing", "Listed Separately", "Sold"] as const;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Inventory"
        description="Track every component, location, assignment, cost, and readiness signal in one place."
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{inventoryInsights(inventory).map((insight) => <div key={insight.title} className={`rounded-2xl border p-4 ${insight.tone === "rose" ? "border-rose-400/20 bg-rose-500/[0.06]" : insight.tone === "amber" ? "border-amber-400/20 bg-amber-500/[0.06]" : "border-sky-400/20 bg-sky-500/[0.06]"}`}><p className="text-sm font-medium text-white">{insight.title}</p><p className="mt-1 text-xs leading-5 text-zinc-400">{insight.detail}</p></div>)}</section>
            ...
            <tbody>
              {visibleItems.map((item) => (
                <tr key={item.id} className="border-b border-white/5 last:border-none">
                  ...
                  <td className="px-4 py-4"><StatusBadge status={item.availability ?? "Unknown"}>{item.availability ?? "Unknown"}</StatusBadge></td>
